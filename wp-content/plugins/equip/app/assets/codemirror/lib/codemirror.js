...
(
	...
	var ie_version = ie && (
		ie_upto10 ? document.documentMode || 6 : ie_11up[1]
	);
		...
		return function ( line ) {
			...
			if ( wrapping ) {
				return widgetsHeight + (
					Math.ceil( line.text.length / perLine ) || 1
				) * th;
			} else {
	...
	NativeScrollbars.prototype = copyObj( {
		update: function ( measure ) {
			...
			if ( needsV ) {
				...
				var totalHeight = measure.viewHeight - (
					needsH ? sWidth : 0
				);
	...
	function updateScrollbarsInner( cm, measure ) {
		...
		d.sizer.style.paddingRight = (
			                             d.barWidth = sizes.right
		                             ) + "px";
		d.sizer.style.paddingBottom = (
			                              d.barHeight = sizes.bottom
		                              ) + "px";
	...
	ContentEditableInput.prototype = copyObj( {
		...
		pollContent: function () {
			...
			if ( from.line == display.viewFrom || (
				                                      fromIndex = findViewIndex( cm, from.line )
			                                      ) == 0 ) {
			...
			var maxCutEnd = Math.min( newBot.length - (
	...
	function findStartLine( cm, n, precise ) {
		...
		var lim = precise ? - 1 : n - (
			cm.doc.mode.innerMode ? 1000 : 100
		);
	...
	function charWidth( display ) {
		...
		var rect = anchor.getBoundingClientRect(), width = (
	...
	function leftButtonDown( cm, e, start ) {
		...
		if ( cm.options.dragDrop && dragAndDrop && ! cm.isReadOnly() &&
		     ...
		     (
			     cmp( (
				     contained = sel.ranges[contained]
			     ).from(), start ) < 0 || start.xRel > 0
		     ) &&
		     (
	...
	function makeChangeSingleDoc( doc, change, selAfter, spans ) {
		...
		if ( change.to.line < doc.first ) {
			shiftDoc( doc, change.text.length - 1 - (
			               change.to.line - change.from.line
			) );
			return;
		}
		...
		if ( change.from.line < doc.first ) {
			var shift = change.text.length - 1 - (
	...
	function makeChangeSingleDocInEditor( cm, change, spans ) {
		...
		var lendiff = change.text.length - (
		              to.line - from.line
		) - 1;
	...
	function calculateScrollPos( cm, x1, y1, x2, y2 ) {
		...
		if ( y1 < screentop ) {
			...
			var newTop = Math.min( y1, (
				                           atBottom ? docBottom : y2
			                           ) - screen );
			if ( newTop != screentop ) {
	...
	function findPosH( doc, pos, dir, unit, visually ) {
		...
		function moveOnce( boundToLine ) {
			var next = (
				visually ? moveVisually : moveLogically
			)( lineObj, ch, dir, true );
			if ( next == null ) {
				if ( ! boundToLine && findNextLine() ) {
					if ( visually ) {
						ch = (
							dir < 0 ? lineRight : lineLeft
						)( lineObj );
					} else {
		...
		indentSelection: methodOp( function ( how ) {
			...
			for ( var i = 0; i < ranges.length; i ++ ) {
				...
				if ( ! range.empty() ) {
					...
					end = Math.min( this.lastLine(), to.line - (
						to.ch ? 0 : 1
					) ) + 1;
		...
		if ( old ) {
			for ( var i = 0, nw; i < old.length; ++ i ) {
				...
				if ( startsBefore || span.from == startCh && marker.type == "bookmark" && (
					...
					).push( new MarkedSpan( marker, span.from, endsAfter ? null : span.to ) );
				}
			}
		}
		return nw;
		...
		if ( old ) {
			for ( var i = 0, nw; i < old.length; ++ i ) {
				...
				if ( endsAfter || span.from == endCh && marker.type == "bookmark" && (
					...
					).push( new MarkedSpan( marker, startsBefore ? null : span.from - endCh,
	...
	function buildLineContent( cm, lineView ) {
		...
		for (
			...
			if ( hasBadBidiRects( cm.display.measure ) && (
					order = getOrder( line )
				) ) {
			...
			if ( nextChange == pos ) { // Update current marker set
				...
				if ( collapsed && (
					...
					buildCollapsedSpan( builder, (
						                             collapsed.to == null ? len + 1 : collapsed.to
					                             ) - pos,
	...
	function moveVisually( line, start, dir, byUnit ) {
		...
		for ( ; ; ) {
			...
			if ( target == part.from || target == part.to ) {
				...
				return (
					       dir > 0
				       ) == part.level % 2 ? part.to : part.from;
			} else {
				part = bidi[pos += dir];
				if ( ! part ) {
					return null;
			...
			return function ( str ) {
				...
				for ( var i = 0; i < len; ++ i ) {
					if ( isNeutral.test( types[i] ) ) {
						...
						var before = (
							             i ? types[i - 1] : outerType
						             ) == "L";
						var after = (
							            end < len ? types[end] : outerType
						            ) == "L";
